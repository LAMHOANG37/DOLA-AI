...
async def _run_task(task_id, model, prompt, ratio, duration, reference_images, client):
    ...
    try:
        ...
        import traceback
        with open("task_error_dump.txt", "w", encoding="utf-8") as f:
...
@app.post("/api/admin/accounts/{name}/cookie")
async def admin_account_update_cookie(name: str, body: AccountCookieUpdate, x_admin_key: str | None = Header(default=None)):
    _admin_auth(x_admin_key)
    try:
        from import_cookies import import_single_account, parse_cookie_input
        content = body.cookie.strip()
        if not content:
            raise HTTPException(400, "Vui lòng nhập chuỗi cookie")

        cookies = parse_cookie_input(content)
        if not cookies:
            raise HTTPException(400, "Không thể đọc định dạng cookie. Vui lòng kiểm tra lại chuỗi cookie.")

        if not any(c.get("name") == "sessionid" for c in cookies):
            raise HTTPException(400, "Cookie thiếu trường 'sessionid'. Vui lòng copy đầy đủ chuỗi cookie từ F12 / Stand.")

        def _run_import(acc, cookie_str):
            import asyncio
            import sys
            ...
            try:
                return loop.run_until_complete(import_single_account(acc, cookie_str))
            finally:
                loop.close()

        import asyncio
        ok = await asyncio.to_thread(_run_import, name, content)
        if ok:
            pool._ensure_meta(name)
            pool.set_login_status(name, True)
            pool.clear_daily_limit(name)
            return {"ok": True, "name": name}
        else:
            raise HTTPException(400, "Đã nạp cookie nhưng trang Dola không nhận diện được phiên đăng nhập (Cookie có thể đã hết hạn trên server Dola).")
        ...
        with open("error_dump.txt", "w", encoding="utf-8") as f:
            traceback.print_exc(file=f)
        raise HTTPException(500, f"Lỗi máy chủ khi nạp cookie: {str(e)}")
...
async def admin_account_import_cookies(body: CookieImportRequest, x_admin_key: str | None = Header(default=None)):
    _admin_auth(x_admin_key)
    try:
        from import_cookies import import_single_account
        ...
        if not lines:
            raise HTTPException(400, "Không tìm thấy cookie hợp lệ")

        imported = []
        failed = []
        existing = pool.accounts
        start_idx = 1
        while f"acc{start_idx}" in existing:
            start_idx += 1
        ...
        for idx, line in enumerate(lines):
            acc_name = f"acc{start_idx + idx}"
            try:
                import asyncio
                ok = await asyncio.to_thread(_run_import, acc_name, line)
                if ok:
                    pool._ensure_meta(acc_name)
                    pool.set_login_status(acc_name, True)
                    pool.clear_daily_limit(acc_name)
                    imported.append(acc_name)
                else:
                    failed.append(acc_name)
            except Exception as e:
                print(f"[import] Error importing {acc_name}: {e}", flush=True)
                failed.append(f"{acc_name} ({str(e)[:100]})")

        if not imported and failed:
            raise HTTPException(400, f"Không thể import: {', '.join(failed)}")

        return {"ok": len(imported) > 0, "imported": imported, "failed": failed}
    except HTTPException:
        raise
    except Exception as e:
        print(f"[import-cookies] Unhandled error: {e}", flush=True)
        raise HTTPException(500, f"Lỗi xử lý cookie: {str(e)}")
...
@app.post("/api/admin/accounts/{name}/login-browser")
async def admin_account_login_browser(name: str, x_admin_key: str | None = Header(default=None)):
    _admin_auth(x_admin_key)
    ...
    from add_account_manual import manual_login
    ...
    def _run_manual():
        ...
        try:
            return loop.run_until_complete(manual_login(name))
        finally:
            loop.close()

    async def _run():
        import asyncio
        JOBS[name] = {"kind": "login_browser", "status": "running", "started_at": time.time()}
        try:
            ok = await asyncio.to_thread(_run_manual)
            if ok:
                pool.set_login_status(name, True)
                JOBS[name] = {**JOBS[name], "status": "success"}
            else:
                JOBS[name] = {**JOBS[name], "status": "failed", "error": "Login timeout or closed"}
        except Exception as e:
...
def _run_add(acc_name, acc_email, acc_pw, acc_totp):
    import sys
    import asyncio as _asyncio
    if sys.platform == "win32":
        _asyncio.set_event_loop_policy(_asyncio.WindowsProactorEventLoopPolicy())
    ...
    _asyncio.set_event_loop(loop)
    try:
        return loop.run_until_complete(add_account_flow(acc_name, acc_email, acc_pw, acc_totp))
    finally:
        loop.close()
...
async def _run_add_job(name: str, email: str, password: str, totp: str):
    ...
    try:
        ok = await asyncio.to_thread(_run_add, name, email, password, totp)
        if ok:
            pool.set_email(name, email)
            pool.set_login_status(name, True)
            JOBS[name] = {**JOBS[name], "status": "success"}
        else:
            JOBS[name] = {**JOBS[name], "status": "failed", "error": "Login failed"}
    except Exception as e:
        JOBS[name] = {**JOBS[name], "status": "failed", "error": str(e)[:300]}
