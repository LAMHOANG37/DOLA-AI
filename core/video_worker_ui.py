...
async def attach_reference_images(page, image_paths: list[str]) -> None:
    ...
    if not await file_input.count():
        upload_btn = page.locator('button[aria-label*="image"], button[aria-label*="upload"], button:has-text("参照"), button:has-text("画像"), svg[class*="upload"]').first
    ...
    try:
        ...
        raise RuntimeError(f"Không tìm thấy ô upload ảnh trên Dola (kiểm tra lại đăng nhập tài khoản): {e}")
    events = []
...
async def generate_video(account: str, prompt: str, ratio: str = None,
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            if login_btn > 0:
                raise AccountLimitedError(f"Tài khoản [{account}] chưa đăng nhập hoặc Cookie phiên đã hết hạn. Vui lòng nạp lại Cookie mới.")

            # ---- UI Submission ----
            video_btn = page.locator('text=動画を作成, text=Create Video, text=Tạo video, text=生成视频').first
            if await video_btn.count():
                await video_btn.click()
            else:
                fallback_btn = page.locator('button:has-text("動画"), button:has-text("Video")').first
