...
class BrowserPool:
    ...
    def clear_daily_limit(self, account: str):
        """Clears the daily limit and quota block for an account (e.g. after importing fresh cookies)."""
        self._conn.execute(
    ...
    async def verify_account(self, name: str) -> bool:
        ...
        def _run_verify():
            ...
            try:
                return loop.run_until_complete(check_login_state(name))
            finally:
                loop.close()
                
        ok = await asyncio.to_thread(_run_verify)
            ...
            async with lock:
                ...
                try:
                    def _run_resume():
                        ...
                        _asyncio.set_event_loop(loop)
                        try:
                            return loop.run_until_complete(resume_video(
                                account, conversation_id, timeout,
                                on_poll=on_poll, on_balance=on_balance))
                        finally:
                            loop.close()

                    result = await asyncio.to_thread(_run_resume)
                    self._claim(account)
                    self._conn.execute(
