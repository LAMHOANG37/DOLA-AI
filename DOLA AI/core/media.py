...
async def _validate_url_async(url: str) -> str:
    ...
    if url.startswith("data:image/"):
        if len(url) > config.REFERENCE_IMAGE_MAX_BYTES * 2:
            raise ValueError("Reference image exceeds file size limit")
        return url
    if len(url) > 4096:
        raise ValueError("Invalid reference image URL")
    parsed = urlparse(url)
    if parsed.scheme.lower() not in {"http", "https"} or not parsed.hostname:
        raise ValueError("Reference image only supports public http/https URLs or base64 data")
    if parsed.username or parsed.password:
        raise ValueError("Reference image URL must not contain authentication credentials")
    host = parsed.hostname
    try:
...
async def download_one_image(session: aiohttp.ClientSession, url: str, dest: Path) -> Path:
    if url.startswith("data:image/"):
        try:
            ...
            if fmt not in _ALLOWED_IMAGE_FORMATS:
                raise ValueError("Reference image only supports JPEG, PNG, WEBP")
            suffix = _ALLOWED_IMAGE_FORMATS[fmt]
            path = dest.with_suffix(suffix)
            path.write_bytes(data)
            return path
        except Exception as exc:
            raise ValueError(f"Failed to decode base64 reference image: {exc}") from exc
